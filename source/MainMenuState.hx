...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		menuBoys = new FlxSprite(750, 100);
		menuBoys.frames = Paths.getSparrowAtlas('menu_ige', 'shared');
		for (i in 0...optionShit.length)
		{
			menuBoys.animation.addByPrefix(optionShit[i], optionShit[i] == 'extras' ? 'extras0' : optionShit[i], 24);
		}
		menuBoys.scrollFactor.set(0, 0);
		menuBoys.antialiasing = true;
		menuBoys.updateHitbox();
		menuBoys.animation.play(optionShit[0]);
		add(menuBoys);
		...
		add(versionShit);
		var versionShit:FlxText = new FlxText(12, FlxG.height - 24, 0, "Internet Golden Era " + Application.current.meta.get('version'), 12);
		versionShit.scrollFactor.set();
		versionShit.setFormat("Comic Sans MS Bold", 16, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		add(versionShit);
	...
	function changeItem(huh:Int = 0)
	{
		...
		menuItems.forEach(function(spr:FlxSprite)
		{
			...
			if (spr.ID == curSelected)
			{
				...
				menuBoys.animation.play(optionShit[curSelected]);
