...
class PlayState extends MusicBeatState
{
	...
	public var haveSeenScreamer:Bool = false;
	public var warningscream:FlxSprite;

	public var aaahhh:FlxSprite;
	...
	override public function create()
	{
		...
		aaahhh = new FlxSprite(0, 0).loadGraphic(Paths.image('jumpscare'));
		aaahhh.alpha = 0;
		add(aaahhh);
		...
		timeBar = new FlxBar(timeBarBG.x + 40000000000, timeBarBG.y + 4, LEFT_TO_RIGHT, Std.int(timeBarBG.width - 8), Std.int(timeBarBG.height - 8), this,
			'songPercent', 0, 1);
		...
		warningscream = new FlxSprite().loadGraphic(Paths.image('dtk1'));
		warningscream.alpha = 0;
		warningscream.screenCenter();
		warningscream.scrollFactor.set();
		add(warningscream);
		warningscream.x += 235;

		if(SONG.song.toLowerCase() == 'unexpected')
		{
			new FlxTimer().start(0, function(tmre:FlxTimer)
			{
				warningscream.loadGraphic(Paths.image('dtk1'));
				new FlxTimer().start(0.2, function(tmr:FlxTimer)
				{
					warningscream.loadGraphic(Paths.image('dtk2'));
					tmre.reset(0.2);
				});
			});
		}

		aaahhh.cameras = [camHUD];
		...
		switch (SONG.song.toLowerCase())
	{
		case 'awesomed up':
			switch (curStep)
			{
				...
				case 822 | 886 | 1078 | 1207 | 1398 | 1526 | 1590 | 1718 | 1783 | 2616 | 2870 | 3126 | 3254:
				    var curZoom = defaultCamZoom;
						var time = (Conductor.stepCrochet / 1000) * 10;
						FlxG.camera.fade(FlxColor.WHITE, time, false, function()
						{
							FlxG.camera.fade(FlxColor.WHITE, 0, true, function()
							{
		...
		if(ratingName == '?') {
			scoreTxt.text = 'Total: ' + songScore + ' | Misses: ' + songMisses + ' | Accuracy: ?';
		} else {
			scoreTxt.text = 'Total: ' + songScore + ' | Misses: ' + songMisses + ' | Accuracy: ' + '' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%' + ' (' + ratingFC + ')';//peeps wanted no integer rating
		}
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (SONG.song.toLowerCase() == 'unexpected')
		{
			...
			if (songMisses % 5 == 0 && haveSeenScreamer == false)
			{
				haveSeenScreamer = true;
				FlxTween.tween(warningscream, {alpha: 1}, 0.2);
				new FlxTimer().start(0, function(tmre:FlxTimer)
				{
					warningscream.loadGraphic(Paths.image('dtk1'));
					new FlxTimer().start(0.2, function(tmr:FlxTimer)
	...
	public function scarypoopoo()
	{
		aaahhh.alpha = 1;
		FlxG.sound.play(Paths.sound('poopoo'));

		FlxTween.tween(aaahhh, {alpha: 0}, 2, {
		startDelay: 3
		});
